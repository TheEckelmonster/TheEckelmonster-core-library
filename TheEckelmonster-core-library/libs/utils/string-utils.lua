...
function string_utils.find_invalid_substrings(string_data)
    _Log.debug("string_utils.find_invalid_substrings")
    _Log.info(string_data)

    return (not string_utils.is_string_valid(string_data)
...
function string_utils.format_surface_name(data)
    -- _Log.debug("string_utils.format_surface_name")
    -- _Log.info(data)
    ...
    local num_loops = 0
    while token do
        if (num_loops > 2 ^ 5) then
            _Log.error("string_utils.format_surface_name looped too many times: num_loops = " .. num_loops); break
        end
...
function string_utils.is_string_valid(string_data)
    _Log.debug("string_utils.is_string_valid")
    _Log.info(string_data)
