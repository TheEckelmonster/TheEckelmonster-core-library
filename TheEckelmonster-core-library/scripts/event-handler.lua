...
function event_handlers.set_log(data)
    if (type(data) ~= "table") then return end
    if (type(data.log) ~= "table") then return end
...
local process_event = function (data)
    ...
    while i <= #data.event_data.order do
        if (loop_count > #data.event_data.order * 1.5) then
            _Log.error("on_event.order processing overran it's order size; breaking")
            break
        end
...
function event_handlers.on_event(event)
    -- _Log.debug("event_handlers.on_event")
    -- _Log.info(event)
...
function event_handlers.on_nth_tick(event)
    -- _Log.debug("event_handlers.on_nth_tick")
    -- _Log.info(event)
...
function event_handlers.on_load()
    _Log.debug("event_handlers.on_load")
...
function event_handlers.on_configuration_changed(event)
    _Log.debug("event_handlers.on_configuration_changed")
    _Log.info(event)
...
function event_handlers.on_init()
    _Log.debug("event_handlers.on_init")
...
function event_handlers:register_event(data)
    _Log.debug("event_handlers:register_event")
    _Log.info(data)
...
function event_handlers:register_events(data_array)
    _Log.debug("event_handlers:register_events")
    _Log.info(data_array)
...
function event_handlers:remove_event(data)
    _Log.debug("event_handlers:remove_event")
    _Log.info(data)
...
function event_handlers:unregister_event(data)
    _Log.debug("event_handlers:unregister_event")
    _Log.info(data)
...
function event_handlers:get_event_position(data)
    _Log.debug("event_handlers:get_event_position")
    _Log.info(data)
...
function event_handlers:set_event_position(data)
    _Log.debug("event_handlers:set_event_position")
    _Log.info(data)
    -- log(serpent.block(data))
...
function event_handlers:on_load_restore(params)
    _Log.debug("event_handlers:on_load_restore")
    _Log.info(params)

    if (not params or type(params) ~= "table") then return end
    if (not params.events or type(params.events) ~= "table") then return end
